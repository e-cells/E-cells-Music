...
const drawerSide = computed(() => isPortrait.value ? 'bottom' : 'right');
const drawerPanelClass = computed(() => isPortrait.value ? 'batch-drawer-bottom' : 'batch-drawer');
...
<template>
  ...
  >
    <div class="batch-drag-handle"></div>
...
:global(.batch-drawer-bottom[data-state='open']) {
...
:global(.batch-drawer-bottom) .batch-drag-handle {
  display: block;
}
