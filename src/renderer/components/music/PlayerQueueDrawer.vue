...
import { useHardwareBack } from '@/composables/useHardwareBack'; // <== 新增：引入返回键拦截器
import {
  iconTrash,
  iconX,
  iconPlay,
...
const contextMenuCloseSignal = ref(0);
watch(
  () => open.value,
  (val) => {
    if (!val) contextMenuCloseSignal.value++;
  },
);

// <== 新增：拦截物理返回键
// 当抽屉处于打开状态时，按下返回键会优先关闭抽屉
                  ...
                  >
                    ...
                    >
                      <SongCard
                        ...
                        :showMore="false"
                        variant="list"
                        :contextMenuCloseSignal="contextMenuCloseSignal"
                      />
                    </div>
