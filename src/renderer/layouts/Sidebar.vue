...
const createdPlaylists = computed(() =>
  playlistStore.getCreatedPlaylists(currentUserIdNumber.value),
);
    ...
    >
      ...
      <div class="px-4 no-drag">
        <div v-for="group in menuGroups" :key="group.title" class="mb-4">
          ...
          <nav class="flex flex-col gap-0.5 relative z-10">
            <Button
      ...
      <div class="pl-7.5 pr-3 mb-2 no-drag -mt-1.5 flex items-center gap-1.5">
      ...
      <nav v-if="isLoggedIn" class="sidebar-scroll-inner flex flex-col gap-0.5 relative z-10">
...
.sidebar {
  /* 移除固定的宽度，交给外部 MainLayout 统一进行自适应控制 */
}
