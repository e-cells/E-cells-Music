...
export const usePlaylistStore = defineStore('playlist', {
  ...
  actions: {
    ...
    getCreatedPlaylists(currentUserId?: number) {
      ...
      const likedPlaylistId = this.likedPlaylist?.id;
      return this.userPlaylists.filter(
        (playlist) =>
          playlist.listCreateUserid === currentUserId &&
          playlist.source !== 2 &&
          playlist.id !== likedPlaylistId && // 排除系统默认红心歌单
          playlist.isDefault !== true && // 排除被标记为系统的默认歌单
          playlist.name !== '默认收藏', // 兜底：强行屏蔽名为“默认收藏”的歌单
      );
    },
    ...
    async favoritePlaylist(meta: PlaylistMeta, currentUserId?: number) {
      ...
      try {
        ...
        if (res && typeof res === 'object' && 'status' in res && res.status === 1) {
          // 乐观更新：先加入本地列表，UI 立即反映
          const data = (res as { data?: unknown }).data;
          let newId: number | undefined;
          if (data && typeof data === 'object' && 'listid' in data) {
            newId = Number((data as { listid?: unknown }).listid);
          }
          this.userPlaylists.push({
            ...meta,
            id: newId ?? meta.id,
            listid: newId ?? meta.listid,
            listCreateListid: meta.listCreateListid ?? meta.id,
            listCreateGid: meta.listCreateGid ?? meta.globalCollectionId,
            source: meta.source ?? 1,
            type: 1,
          });
          // 不调用 fetchUserPlaylists()，避免服务端延迟导致乐观更新被覆盖
          return true;
        }
      } catch (e) {
    ...
    async unfavoritePlaylist(meta: PlaylistMeta, currentUserId?: number) {
      // 乐观更新：先找到目标并从本地列表中移除
      const targetIndex = this.userPlaylists.findIndex((playlist) => {
      ...
      const target = this.userPlaylists[targetIndex];
      const listId = target?.listid ?? target?.id;
      if (!listId) return false;

      // 乐观更新：先从本地列表中移除，UI 立即反映
      const removed = this.userPlaylists.splice(targetIndex, 1)[0];
      ...
      try {
        ...
        // API 失败：回滚
        this.userPlaylists.splice(targetIndex, 0, removed);
      } catch (e) {
        // 异常：回滚
        this.userPlaylists.splice(targetIndex, 0, removed);
