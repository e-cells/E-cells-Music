    ...
    try {
      ...
      while (keepGoing && !this._aborted && nextPage <= this.maxPages) {
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          // 🛠️ 关键修复 2：用“连续重复次数”替代“单次去重无新增就直接中断”的逻辑
          if (deduped.length > 0) {
            this.onPageLoaded?.(this._items, deduped, page);
            consecutiveDuplicates = 0; // 有新数据则重置计数
          } else if (items.length > 0) {
            consecutiveDuplicates++;   // 请求成功但全是重复数据，计数累加
          }

          // 终止条件：API 返回空数据、连续3页全都是重复数据、或 fetcher 标记无更多页
          if (items.length === 0 || consecutiveDuplicates >= 3 || !hasMore) {
            keepGoing = false;
            break;
          }
        }
