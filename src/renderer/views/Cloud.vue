  ...
  <div class="cloud-view bg-bg-main min-h-full">
    ...
    <template v-else>
      ...
      <template v-if="!isPortrait">
        <SliverHeader typeLabel="CLOUD" title="音乐云盘" :coverUrl="cloudCoverUrl" :hasDetails="true">
          <template #details>
            <div class="flex flex-col gap-2 w-full items-start text-left">
        ...
        <div class="song-list-sticky sticky z-[110] bg-bg-main" :style="{ top: '56px' }">
          ...
          <SongListHeader :sortField="sortField" :sortOrder="sortOrder" :showCover="true" paddingClass="px-6" @sort="handleSort" />
        </div>

        <div class="px-6 pb-12 relative z-[1]">
          <div v-if="loading" class="flex items-center justify-center py-20">
            <div class="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
          </div>
          <div v-else-if="songs.length === 0" class="cloud-empty flex flex-col items-center justify-center py-24 text-center">
          ...
          <SongList v-else ref="songListRef" :songs="sortedSongs" :searchQuery="searchQuery" :activeId="activeSongId" :showCover="true" :queueOptions="{ queueId: 'queue:cloud', title: '云盘音乐', subtitle: '你的云盘收藏', type: 'cloud', dynamic: false }" :enableDefaultDoubleTapPlay="true" :onSongDoubleTapPlay="settingStore.replacePlaylist ? handleSongDoubleTapPlay : undefined" />
      ...
      <template v-else>
        ...
        <div class="px-4 pt-3">
          <div class="flex items-center gap-3">
            <div class="w-14 h-14 rounded-[18px] bg-primary/10 text-primary flex items-center justify-center shrink-0">
              <Icon :icon="iconCloud" width="26" height="26" />
            </div>
            <div class="min-w-0 flex-1">
              <div class="text-[16px] font-extrabold text-text-main">音乐云盘</div>
              <div class="flex items-center gap-3 mt-0.5 text-[11px] text-text-secondary/80">
                ...
                <span class="inline-flex items-center gap-1">
                  <Icon :icon="iconCloud" width="11" height="11" />
                  {{ formatBytes(cloudCapacity) }}
                </span>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-1.5 mt-2.5">
            <span class="px-2 py-0.5 rounded text-[10px] font-bold tracking-[0.8px] uppercase text-primary bg-primary/10 border border-primary/15">CLOUD</span>
          </div>
          ...
          <div class="mt-2.5 pb-1">
            <div class="cloud-info-card">
              <div class="flex items-center justify-between">
                <div class="text-[12px] font-semibold text-text-main">容量</div>
                <div class="text-[11px] font-semibold text-primary">{{ (usageRatio * 100).toFixed(1) }}%</div>
              </div>
              <div class="cloud-progress-track">
                <div class="cloud-progress-value" :style="{ width: `${usageRatio * 100}%` }"></div>
              </div>
              <div class="flex items-center justify-between text-[10px] font-medium text-text-secondary/70">
                <span>{{ formatBytes(usedCapacity) }} / {{ formatBytes(cloudCapacity) }}</span>
                <span>可用 {{ formatBytes(cloudAvailable) }}</span>
              </div>
            </div>
        ...
        <div class="pb-safe relative z-[1]">
          ...
          <div v-else class="px-3">
            <SongList ref="songListRef" :songs="sortedSongs" :searchQuery="searchQuery" :activeId="activeSongId" :showCover="true" :queueOptions="{ queueId: 'queue:cloud', title: '云盘音乐', subtitle: '你的云盘收藏', type: 'cloud', dynamic: false }" :enableDefaultDoubleTapPlay="true" :onSongDoubleTapPlay="settingStore.replacePlaylist ? handleSongDoubleTapPlay : undefined" />
          </div>
          <div v-if="!loading && isBackgroundResolving" class="flex justify-center pt-4">
            <div class="text-[12px] font-semibold text-text-secondary/70">正在后台补全剩余云盘歌曲...</div>
          </div>
