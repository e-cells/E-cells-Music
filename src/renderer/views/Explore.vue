...
// UI 响应式偏移
const headerHeight = computed(() => (isGeckoView ? 0 : props.showHeader ? 102 : 0));
const rankToolbarOffset = computed(() => headerHeight.value + (isGeckoView ? 52 : 46));
const newSongToolbarOffset = computed(() => headerHeight.value + (isGeckoView ? 52 : 46));
...
const loadRecommendedPlaylists = async () => {
  ...
  try {
    const res = await getPlaylistByCategory(playlistCategoryId.value || '0', 0, 100);
...
<template>
  ...
  >
    ...
    <div v-if="activeTabIndex === 0" class="mt-2">
      <div class="explore-toolbar sticky z-[120] bg-bg-main/80 backdrop-blur-xl py-2 -mx-4 px-4 md:-mx-0 md:px-0" :style="{ top: showHeader ? '102px' : '0' }">
      ...
      <div v-else :class="['mt-4', showHeader ? 'pb-safe' : 'pb-4']">
        ...
        >
          <template #default="{ item }">
            <PlaylistCard v-bind="item" layout="list" />
          </template>
        </VirtualGrid>
      </div>
    </div>
    ...
    <div v-else-if="activeTabIndex === 1" class="mt-0">
      <div class="rank-toolbar sticky z-[120] bg-bg-main/80 backdrop-blur-xl -mx-4 px-4 md:-mx-0 md:px-0" :style="{ top: showHeader ? '102px' : '0' }">
        <div class="rank-toolbar-inner">
          <CustomSelector :label="rankLabel" @click="showRankPicker = true" />
          <div class="rank-toolbar-actions">
            <div class="rank-action-scroll scrollbar-hide">
      ...
      <div
        class="song-list-sticky sticky z-[110] bg-bg-main/90 backdrop-blur-xl border-b border-border-light/10 -mx-4 px-4 md:-mx-0 md:px-0"
        :style="{ top: `${rankToolbarOffset}px` }"
      >
        <div class="flex items-center justify-between h-14">
          <div class="rank-song-tab">
            <span class="rank-song-label relative">歌曲 <Badge :count="rankSongCountLabel" class="scale-90 origin-left" /></span>
          </div>
          <div class="flex items-center gap-2">
            <div class="relative hidden sm:block">
              <input
                ...
                type="text"
                placeholder="搜索榜单内歌曲..."
                class="song-search-input w-48 h-9 pl-8 pr-3 rounded-full bg-black/5 dark:bg-white/10 border border-transparent shadow-none text-text-main placeholder:text-text-main/50 outline-none text-[13px] transition-all focus:bg-black/10 dark:focus:bg-white/15"
              />
              <Icon class="absolute left-3 top-1/2 -translate-y-1/2 text-text-main/50" :icon="iconSearch" width="14" height="14" />
            </div>
            <Button
              variant="unstyled"
              size="none"
    ...
    <div v-else-if="activeTabIndex === 2" class="mt-2">
      <div class="explore-toolbar sticky z-[120] bg-bg-main/80 backdrop-blur-xl py-2 -mx-4 px-4 md:-mx-0 md:px-0" :style="{ top: showHeader ? '102px' : '0' }">
        <CustomSelector :label="albumTypeLabel" @click="showAlbumPicker = true" />
      </div>
      <div v-if="loadingAlbums" class="flex items-center justify-center py-20">
        <div class="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
      </div>
    ...
    <div v-else-if="activeTabIndex === 3" class="mt-0">
      <div class="new-song-toolbar sticky z-[120] bg-bg-main/80 backdrop-blur-xl -mx-4 px-4 md:-mx-0 md:px-0" :style="{ top: showHeader ? '102px' : '0' }">
        <div class="new-song-toolbar-inner">
          <div class="new-song-title-wrap">
            <div class="new-song-badge-icon shadow-sm">
              <Icon :icon="iconSparkles" width="16" height="16" />
            </div>
            <div class="min-w-0">
              <div class="text-[15px] font-bold text-text-main leading-none">新歌速递</div>
            </div>
          </div>
      ...
      <BatchActionDrawer v-model:open="showNewSongBatchDrawer" :songs="newSongs" source-id="new-song" />
      ...
      >
        <div class="flex items-center justify-between h-14">
          <div class="rank-song-tab">
            <span class="rank-song-label relative">歌曲 <Badge :count="newSongCountLabel" class="scale-90 origin-left" /></span>
          </div>
          <div class="flex items-center gap-2">
            <div class="relative hidden sm:block">
              <input
    ...
    <div v-else-if="activeTabIndex === 4" class="mt-2">
      <div class="explore-toolbar sticky z-[120] bg-bg-main/80 backdrop-blur-xl py-2 -mx-4 px-4 md:-mx-0 md:px-0" :style="{ top: showHeader ? '102px' : '0' }">
      ...
      <template v-else-if="filteredArtistCards.length > 0">
        <div class="artist-letter-bar sticky z-[110] bg-bg-main/90 backdrop-blur-xl -mx-4 px-4 md:-mx-0 md:px-0 border-b border-border-light/10 pb-3" :style="{ top: `${rankToolbarOffset}px` }">
          <span
      ...
      <div v-else class="py-20 text-center opacity-50 text-[14px]">暂无歌手</div>
    </div>

    <CustomPicker v-model:open="showArtistSexPicker" title="性别筛选" :options="artistSexTypes" :selectedId="artistSexId" @select="handleSelectArtistSex" :maxWidth="360" />
    <CustomPicker v-model:open="showArtistTypePicker" title="类型筛选" :options="artistTypes" :selectedId="artistTypeId" @select="handleSelectArtistType" :maxWidth="360" />
    <CustomPicker v-model:open="showPlaylistPicker" title="歌单分类" :options="playlistCategories" :selectedId="playlistCategoryId" @select="handleSelectPlaylistCategory" />
    <CustomPicker v-model:open="showRankPicker" title="排行榜选择" :options="ranks.map((rank) => ({ id: String(rank.id), name: rank.name }))" :selectedId="rankId ? String(rankId) : ''" @select="handleSelectRank" />
    <CustomPicker v-model:open="showAlbumPicker" title="专辑类型" :options="albumTypes" :selectedId="albumTypeId" @select="handleSelectAlbumType" :maxWidth="360" />
  </div>
</template>
...
.dark .artist-letter-item {
  background: rgba(255, 255, 255, 0.05);
}

.artist-letter-item:active {
