  ...
  <div class="history-view bg-bg-main min-h-full">
    ...
    <template v-else>
      ...
      <template v-if="!isPortrait">
        <SliverHeader typeLabel="HISTORY" title="最近播放" :coverUrl="historyCoverUrl" :hasDetails="true">
          <template #details>
        ...
        <div class="px-6 pb-12 relative z-[1]">
          ...
          <div v-else-if="songs.length === 0" class="history-empty flex flex-col items-center justify-center py-24 text-center">
          ...
          <SongList v-else ref="songListRef" :songs="sortedSongs" itemKeyField="historyKey" :searchQuery="searchQuery" :activeId="activeSongId" :showCover="true" :queueOptions="{ queueId: 'queue:history', title: '播放历史', subtitle: '最近播放记录', type: 'history', dynamic: false }" :enableDefaultDoubleTapPlay="true" :onSongDoubleTapPlay="settingStore.replacePlaylist ? handleSongDoubleTapPlay : undefined" />
          <div v-if="!loading && hasMore" class="flex justify-center pt-4">
            <Button variant="unstyled" size="none" class="px-4 h-9 rounded-lg bg-black/[0.04] dark:bg-white/[0.06] text-[12px] font-semibold text-text-main/75 hover:text-text-main transition-colors" :disabled="loadingMore" @click="handleLoadMore">
      ...
      <template v-else>
          ...
          <div class="border-b border-border-light/10 px-4">
            <div class="flex items-center justify-between h-10">
              <div class="text-[13px] font-semibold text-text-main relative">
                歌曲 <Badge :count="displayedCountLabel" class="scale-90 origin-left" />
              </div>
        ...
        <div class="pb-safe relative z-[1]">
          ...
          <div v-else class="px-3">
            <SongList ref="songListRef" :songs="sortedSongs" itemKeyField="historyKey" :searchQuery="searchQuery" :activeId="activeSongId" :showCover="true" :queueOptions="{ queueId: 'queue:history', title: '播放历史', subtitle: '最近播放记录', type: 'history', dynamic: false }" :enableDefaultDoubleTapPlay="true" :onSongDoubleTapPlay="settingStore.replacePlaylist ? handleSongDoubleTapPlay : undefined" />
          </div>
          <div v-if="!loading && hasMore" class="flex justify-center pt-4">
            <Button variant="unstyled" size="none" class="px-4 h-9 rounded-lg bg-black/[0.04] dark:bg-white/[0.06] text-[12px] font-semibold text-text-main/75 hover:text-text-main transition-colors" :disabled="loadingMore" @click="handleLoadMore">
              {{ loadingMore ? '加载中...' : '加载更多' }}
