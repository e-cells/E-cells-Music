...
<template>
  ...
  >
    ...
    <div class="absolute inset-x-0 bottom-0 top-14 z-10 flex flex-col overflow-hidden">
      <div
        class="px-4 md:px-6 pb-2 md:pb-3 no-drag transition-opacity duration-500"
        :style="{
      ...
      >
        <div class="flex h-12 items-center w-full">
          ...
          <div class="ml-auto flex items-center gap-2 overflow-x-auto overflow-y-hidden scrollbar-hide shrink-0 max-w-[calc(100vw-80px)] pr-2">
            <div
              v-if="hasPortraitGallery && artistPortraitUrls.length > 1"
              class="lyric-tool-group shrink-0"
              title="切换歌手写真"
            >
              <Button
                variant="unstyled"
            ...
            >
              <template #trigger>
                <Button
                  ...
                  size="none"
                  type="button"
                  class="lyric-tool-chip shrink-0"
                  title="背景透明度"
                >
            ...
            >
              <template #trigger>
                <Button variant="unstyled" size="none" type="button" class="lyric-tool-chip shrink-0">
                  <FontIcon :size="14" />
                  <span>字体</span>
                </Button>
              </template>
      ...
      <div class="flex-1 min-h-0 px-4 md:px-6 pb-2 no-drag">
        <div class="mx-auto flex h-full w-full max-w-[1560px] flex-col md:flex-row gap-2 md:gap-7">
          
          <section
            v-if="!hasPortraitGallery"
            class="flex min-w-[160px] shrink-0 flex-col items-center justify-center md:max-w-[420px] md:flex-[5] pt-4 md:pt-0"
          >
            <div class="lyric-info-card lyric-info-panel cursor-pointer" @click.stop="copySongInfo">
              <div class="lyric-cover-shell mx-auto">
                <!-- 经典方形 -->
                <div v-if="coverStyle === 'square'" class="lyric-cover-frame">
                  <Cover
                ...
                <div
                  ...
                  class="lyric-disc-cover dvd-spin"
                  :class="{ 'is-paused': !isPlaying }"
                >
                  <div class="lyric-disc-rim"></div>
                  <Cover
                    :url="currentTrack?.coverUrl"
                    :size="800"
                    :borderRadius="'50%'"
                  ...
                  />
                  <div class="lyric-disc-hole"></div>
                </div>

                <!-- 呼吸脉动 -->
                <div
                  v-else-if="coverStyle === 'breathing'"
                  class="lyric-breathing-cover"
                  :class="{ 'is-paused': !isPlaying }"
                >
                  <Cover
                    :url="currentTrack?.coverUrl"
              ...
              <div class="mt-4 md:mt-7 w-full max-w-[420px] space-y-1 md:space-y-2 text-center">
                ...
                <p
                  ...
                  class="inline-block rounded-full bg-yellow-600/15 dark:bg-yellow-400/10 px-2 py-1 text-[11px] text-yellow-700 dark:text-yellow-400 pointer-events-none mt-1"
                >
                  时间存疑，歌词可能不同步
                </p>
              </div>
          ...
          <section
            class="lyric-panel-surface relative flex min-w-0 flex-col justify-center self-stretch flex-1 md:flex-[7]"
          >
            <div
              v-if="hasPortraitGallery && currentTrack"
              class="absolute left-4 md:left-6 bottom-2 z-30 flex flex-col items-start gap-1.5 transition-opacity duration-500"
              :style="{
                opacity: isLyricCollapsed ? 0 : 1,
                pointerEvents: isLyricCollapsed ? 'none' : undefined,
              }"
            ...
            >
              ...
              >
                ...
                <div v-else class="flex h-full items-center justify-center text-center">
                  <div class="space-y-3">
                    <p class="text-[24px] md:text-[28px] font-semibold text-black/88 dark:text-white/88">
                      {{ emptyStateTitle }}
                    </p>
                  </div>
                </div>
      ...
      <div
        class="px-4 md:px-6 pb-2 pt-2 md:pt-0 no-drag transition-opacity duration-500"
        :style="{
          opacity: isLyricCollapsed ? 0 : 1,
          pointerEvents: isLyricCollapsed ? 'none' : undefined,
        }"
      >
        <div class="lyric-controls-surface mx-auto flex w-full max-w-[820px] flex-col gap-2 md:gap-0.5">
          <div
            class="w-full flex flex-col md:grid md:grid-cols-[1fr_minmax(0,420px)_1fr] items-center gap-3 md:gap-4"
          >
            <div class="min-w-0 flex items-center gap-2 w-full order-1 md:order-2">
              <span
              ...
              <SliderRoot
                ...
                :step="1"
                class="relative flex items-center select-none touch-none flex-1 min-w-0 h-8 md:h-4 group/progress cursor-pointer"
              ...
              >
                <SliderTrack
                  class="bg-black/10 dark:bg-white/10 relative grow rounded-full h-[4px] md:h-[3px]"
                >
                  ...
                  <div
                    v-if="playerStore.cacheProgress > 0"
                    class="absolute rounded-full h-full bg-black/20 dark:bg-white/25 transition-[width] duration-300"
                    :style="{ width: (playerStore.cacheProgress * 100).toFixed(1) + '%' }"
                  ></div>
                  <SliderRange class="absolute bg-black dark:bg-white rounded-full h-full" />
                </SliderTrack>
                <SliderThumb
                  class="block w-3.5 h-3.5 md:w-3 md:h-3 bg-black dark:bg-white rounded-full shadow-md focus-visible:outline-none transition-[opacity,transform] duration-200 opacity-100 md:opacity-0 md:group-hover/progress:opacity-100 scale-100 md:scale-50 md:group-hover/progress:scale-110"
                />
              </SliderRoot>
              <span
            ...
            <div class="flex items-center justify-center md:justify-start gap-4 md:gap-0 w-full md:w-auto order-2 md:order-1">
              ...
              <Tooltip v-if="canAddToPlaylist" content="添加到" side="top">
                <template #trigger>
                  ...
                  >
                    <Icon :icon="iconPlaylistAdd" width="22" height="22" />
                  </Button>
                </template>
              </Tooltip>
            ...
            <div class="flex items-center justify-center md:justify-end gap-3 md:gap-1 select-none w-full md:w-auto order-3 md:order-3 pb-2 md:pb-0">
          ...
          <div class="lyric-controls-row flex items-center justify-center gap-6 md:gap-5 mt-2 md:-mt-6 z-10 pointer-events-none">
            <div class="pointer-events-auto flex items-center gap-6 md:gap-5">
              <Tooltip :content="playModeLabel" side="top">
                <template #trigger>
                  <Button
                    ...
                    size="none"
                    type="button"
                    class="flex h-10 w-10 md:h-9 md:w-9 items-center justify-center rounded-full transition-all hover:scale-110 active:scale-90"
                    @click="cyclePlayMode"
                  >
                    <Icon
                      :icon="playModeIcon"
              ...
              <Button
                ...
                size="none"
                type="button"
                class="flex h-12 w-12 md:h-10 md:w-10 items-center justify-center rounded-full transition-all hover:scale-110 active:scale-90"
                title="上一曲"
              ...
              <Button
                ...
                size="none"
                type="button"
                class="lyric-main-play-btn flex h-16 w-16 md:h-12 md:w-12 items-center justify-center rounded-full"
              ...
              <VolumePopover variant="lyric" class="scale-110 md:scale-100" />
            </div>
          </div>
...
.lyric-disc-cover.dvd-spin {
  animation: dvd-rotate 24s linear infinite;
}

.lyric-disc-cover.dvd-spin.is-paused {
  animation-play-state: paused;
}

/* ── 横屏呼吸封面 ── */
.lyric-breathing-cover {
  position: relative;
  width: 100%;
  height: 100%;
  border-radius: 50%;
  border: 1px solid color-mix(in srgb, var(--color-text-main) 10%, transparent);
  animation: lyric-breathing 4s ease-in-out infinite;
  will-change: transform, box-shadow;
}

.lyric-breathing-cover.is-paused {
  animation-play-state: paused;
}

@keyframes lyric-breathing {
  0% {
