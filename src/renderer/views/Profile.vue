...
const followCount = computed(() => {
  const d = detail.value as Record<string, any>;
  const u = userInfo.value as Record<string, any>;
  return d?.follows ?? d?.following ?? d?.following_count ?? u?.follows ?? u?.following ?? 0;
});

const fansCount = computed(() => {
  const d = detail.value as Record<string, any>;
  const u = userInfo.value as Record<string, any>;
  return d?.fans ?? d?.fans_count ?? d?.followers ?? d?.follower_count ?? u?.fans ?? u?.followeds ?? 0;
});

const visitorCount = computed(() => {
  const d = detail.value as Record<string, any>;
  return d?.nvisitors ?? d?.visitor_count ?? d?.visitors ?? 0;
});
// ========================================
            ...
            <div class="flex items-center gap-6 relative z-10">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-col">
                  <p
                    v-if="detail.descri"
                    class="text-[12px] opacity-70 font-medium line-clamp-2 mb-2"
                  >
                    {{ detail.descri }}
                  </p>
                  ...
                  <div class="flex items-center gap-5 mt-1.5">
                    <div class="flex flex-col items-center justify-center min-w-[36px]">
                      <span class="text-[16px] font-black leading-tight">{{ detail.p_grade || 0 }}</span>
                      <span class="text-[10px] opacity-60 uppercase font-bold tracking-wider mt-0.5">等级</span>
                    </div>
                    <div class="w-[1px] h-4 bg-black/10 dark:bg-white/10"></div>
                    
                    <div class="flex flex-col items-center justify-center min-w-[36px]">
                      <span class="text-[16px] font-black leading-tight">{{ followCount }}</span>
                      <span class="text-[10px] opacity-60 uppercase font-bold tracking-wider mt-0.5">关注</span>
                    </div>
                    <div class="w-[1px] h-4 bg-black/10 dark:bg-white/10"></div>
                    
                    <div class="flex flex-col items-center justify-center min-w-[36px]">
                      <span class="text-[16px] font-black leading-tight">{{ fansCount }}</span>
                      <span class="text-[10px] opacity-60 uppercase font-bold tracking-wider mt-0.5">粉丝</span>
                    </div>
                    <div class="w-[1px] h-4 bg-black/10 dark:bg-white/10"></div>
                    
                    <div class="flex flex-col items-center justify-center min-w-[36px]">
                      <span class="text-[16px] font-black leading-tight">{{ visitorCount }}</span>
                      <span class="text-[10px] opacity-60 uppercase font-bold tracking-wider mt-0.5">访客</span>
                    </div>
                  </div>
                </div>
              </div>
