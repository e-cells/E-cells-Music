...
// 响应式顶部高度偏移
const pinnedTabHeight = isGeckoView ? 48 : 50;
...
<template>
  <div class="search-view relative pb-10 min-h-screen bg-bg-main">
      ...
      <div class="search-input-shell mt-4" :class="{ 'has-suggestions': showSuggestions }">
        <div class="search-input-wrap shadow-sm">
          ...
          <input
            ...
            type="text"
            class="search-input"
            :placeholder="defaultKeyword ? `大家都在搜: ${defaultKeyword}` : '搜索音乐、歌手、专辑'"
        ...
        <div v-if="showSuggestions" class="search-suggestions-panel backdrop-blur-2xl bg-bg-sidebar/95 shadow-2xl">
          <div
              ...
              >
                ...
                >
                  <span class="search-suggestion-leading">
                    <Icon :icon="iconSearch" width="14" height="14" class="opacity-40" />
                  </span>
      ...
      <template v-else>
        <div v-if="searchHistory.length > 0" class="search-section mt-2">
          ...
          <div class="search-chip-wrap">
            ...
            >
              <span class="truncate max-w-[120px]">{{ keyword }}</span>
              <span
                class="history-chip-close ml-1"
        ...
        <div class="search-section mt-8">
          ...
          <div v-if="hotSearchCategories.length > 0" class="search-hot-tabs mt-3 scrollbar-hide">
            <Button
              variant="unstyled"
              size="none"
          ...
          <div class="search-chip-wrap mt-4">
            <Button
              variant="unstyled"
              size="none"
              v-for="(item, idx) in currentHotKeywords"
              :key="`${item.keyword}-${item.reason}`"
              type="button"
              class="hot-chip"
              :class="{'text-red-500 font-bold border-red-500/20': idx < 3 && selectedHotCategoryIndex === 0}"
              @click="runSearch(item.keyword)"
            >
              <span>{{ item.keyword }}</span>
              <template v-if="item.reason && item.reason !== item.keyword">
                <span class="opacity-30 mx-0.5">|</span>
                <span class="hot-chip-reason opacity-70">{{ item.reason }}</span>
              </template>
            </Button>
          </div>
        </div>
    ...
    <div v-else class="px-0 md:px-10 pt-2">
      <div v-if="activeTabIndex === 0">
        <div
          class="search-song-toolbar sticky z-[120] backdrop-blur-xl bg-bg-main/80 border-b border-border-light/10 px-4 md:px-0"
          :style="{ top: `${songToolbarOffset}px` }"
        >
          <div class="search-song-toolbar-inner">
            <div class="search-song-title-wrap">
              <div class="search-song-badge-icon bg-gradient-to-br from-primary to-secondary shadow-md">
                <Icon :icon="iconSparkles" width="16" height="16" />
              </div>
              <div class="text-[15px] font-bold text-text-main leading-none">搜索结果</div>
            </div>
        ...
        <div class="pb-12 px-2 md:px-0 mt-2">
          <SongList
            ref="songListRef"
          ...
          >
            {{ activePagination.loadingMore ? '正在加载更多内容...' : '继续下滑加载' }}
          </div>
          <div
      ...
      <div v-else-if="activeTabIndex === 1" class="px-4 md:px-0">
        <VirtualGrid
          class="pb-6"
          :items="playlistCards"
        ...
        <div v-if="activePagination.loadingMore || activePagination.hasMore" class="search-load-more-status">
          {{ activePagination.loadingMore ? '正在加载...' : '继续下滑' }}
        </div>
      </div>

      <div v-else-if="activeTabIndex === 2" class="px-4 md:px-0">
        <VirtualGrid
          class="pb-6"
          :items="albumCards"
      ...
      <div v-else-if="activeTabIndex === 3" class="px-4 md:px-0">
        <VirtualGrid
          class="pb-6 mt-2"
          :items="artistCards"
      ...
      <div v-else-if="activeTabIndex === 4">
        ...
        >
          <div class="search-song-toolbar-inner">
            <div class="search-song-title-wrap">
              <div class="search-song-badge-icon bg-gradient-to-br from-primary to-secondary">
                <Icon :icon="iconSparkles" width="16" height="16" />
              </div>
              <div class="text-[15px] font-bold text-text-main leading-none">包含歌词</div>
            </div>
            <div class="search-song-toolbar-actions">
              <div class="overflow-x-auto scrollbar-hide">
        ...
        <BatchActionDrawer v-model:open="showLyricBatchDrawer" :songs="lyricResults" source-id="search-lyric" />
...
.dark .search-input-wrap:focus-within {
...
.search-input::placeholder {
  color: color-mix(in srgb, var(--color-text-main) 35%, transparent);
  font-weight: 400;
}
...
.hot-chip {
  ...
  align-items: center;
  min-height: 34px;
  border-radius: 10px; /* 胶囊圆角变方形圆角，更大气 */
