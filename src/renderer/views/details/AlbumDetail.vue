...
<template>
  <div class="album-detail-container bg-bg-main min-h-full">
    ...
    <template v-else-if="album">
      ...
      <template v-if="!isPortrait">
        ...
        >
            ...
            <div class="flex flex-col gap-1.5 text-text-main/60">
              <div class="album-artist-line">
                <template v-for="(artistItem, index) in albumArtists" :key="`${artistItem.name}-${index}`">
                  <Button variant="unstyled" size="none" type="button" class="album-singer-link" :class="{ 'is-link': isAlbumArtistClickable(artistItem) }" :disabled="!isAlbumArtistClickable(artistItem)" @click="openAlbumArtist(artistItem)">
                    {{ artistItem.name }}
                  </Button>
              ...
              <div v-if="album.type || album.language" class="text-[11px] font-semibold text-text-secondary">
          ...
          <template #collapsed-actions>
            <Button v-if="userStore.isLoggedIn" variant="unstyled" size="none" @click="toggleFavoriteAlbum" class="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 text-red-500">
        ...
        <div v-if="album.intro" class="px-6 pt-[6px] pb-[6px]">
          ...
          <div class="mt-[6px] text-[12px] leading-relaxed text-text-secondary line-clamp-1">{{ album.intro }}</div>
          <Button variant="unstyled" size="none" type="button" class="mt-[2px] text-[11px] font-semibold text-primary" @click="showIntroDialog = true">查看详情</Button>
        </div>
              ...
              <div class="flex items-center justify-between h-14">
                <TabsList class="bg-transparent border-none gap-8">
                  ...
                  <TabsTrigger value="comments">
                    <span class="relative">评论 <Badge v-if="commentTotal > 0" :count="commentTotal" class="-right-6" /></span>
                  </TabsTrigger>
                </TabsList>
                <div v-if="activeTab === 'songs'" class="flex items-center gap-2">
                  <div class="relative">
                    <input v-model="searchQuery" type="text" placeholder="搜索歌曲..." class="song-search-input w-52 h-9 pl-8 pr-3 rounded-lg bg-white border border-black/30 shadow-sm text-text-main placeholder:text-text-main/50 dark:bg-white/[0.08] dark:border-white/10 dark:shadow-none outline-none text-[12px] transition-all" />
                    <Icon class="absolute left-2.5 top-1/2 -translate-y-1/2 text-text-main/60" :icon="iconSearch" width="14" height="14" />
                  </div>
                  <Button variant="unstyled" size="none" @click="handleLocate" class="song-locate-btn p-2 rounded-lg" title="定位当前播放">
                    <Icon :icon="iconCurrentLocation" width="18" height="18" />
                  </Button>
                </div>
              </div>
            </div>
            <SongListHeader v-if="activeTab === 'songs'" :sortField="sortField" :sortOrder="sortOrder" :showCover="true" paddingClass="px-6" @sort="handleSort" />
          </div>

          <div class="pb-12 relative z-[1]">
            <TabsContent value="songs" class="px-6 flex flex-col flex-1 min-h-0">
              <SongList ref="songListRef" :songs="sortedSongs" :loading="loadingSongs" :active="activeTab === 'songs'" :searchQuery="searchQuery" :activeId="activeSongId" :showCover="true" :queueOptions="{ queueId: `queue:album:${album?.id ?? getAlbumId()}`, title: album?.albumname || '专辑', subtitle: album?.singerName || '', type: 'album' }" :enableDefaultDoubleTapPlay="true" :onSongDoubleTapPlay="settingStore.replacePlaylist ? handleSongDoubleTapPlay : undefined" />
            </TabsContent>
            <TabsContent value="comments" class="px-6 pt-5 pb-10">
              <div class="w-full">
                ...
                <CommentList :comments="hotComments" :loading="loadingComments" resourceType="album" :fallbackMixSongId="String(currentId)" compact hide-empty />
                <CommentList :comments="comments" :loading="loadingComments" :total="commentTotal" resourceType="album" :fallbackMixSongId="String(currentId)" compact :hide-empty="hotComments.length > 0" />
                <div v-if="loadingComments || ((hotComments.length > 0 || comments.length > 0) && !hasMoreComments)" class="flex justify-center mt-8">
                  <div class="text-[12px] font-semibold text-text-secondary">{{ loadingComments ? '加载中...' : '已加载全部评论' }}</div>
                </div>
              </div>
            </TabsContent>
          </div>
      ...
      <template v-else>
        ...
        <div class="px-4 pt-3">
          <div class="flex items-start gap-3">
            <div class="w-14 h-14 rounded-xl overflow-hidden shadow-lg shrink-0">
              <Cover :url="album.pic" :size="160" :width="56" :height="56" />
            </div>
            <div class="min-w-0 flex-1 pt-0.5">
              <div class="text-[16px] font-extrabold text-text-main leading-snug line-clamp-2">{{ album.name }}</div>
              <div class="flex items-center gap-1 mt-1 flex-wrap">
                <template v-for="(artistItem, index) in albumArtists" :key="`${artistItem.name}-${index}`">
                  <Button variant="unstyled" size="none" type="button" class="album-singer-link text-[12px]" :class="{ 'is-link': isAlbumArtistClickable(artistItem) }" :disabled="!isAlbumArtistClickable(artistItem)" @click="openAlbumArtist(artistItem)">
                    {{ artistItem.name }}
                  </Button>
                  <span v-if="index < albumArtists.length - 1" class="album-artist-separator text-[11px]">/</span>
                </template>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center flex-wrap gap-1.5 mt-2.5">
            <span class="px-2 py-0.5 rounded text-[10px] font-bold tracking-[0.8px] uppercase text-primary bg-primary/10 border border-primary/15">ALBUM</span>
            <span class="text-[11px] font-semibold text-text-main/60">{{ album.publishTime }}</span>
            <span class="text-[11px] font-semibold text-text-main/40">•</span>
            <span class="text-[11px] font-semibold text-text-main/60">{{ albumSongCount }} 首</span>
            <template v-if="album.type">
              <span class="text-[11px] font-semibold text-text-main/40">•</span>
              <span class="text-[11px] font-semibold text-text-main/60">{{ album.type }}</span>
            </template>
            <template v-if="album.language">
              <span class="text-[11px] font-semibold text-text-main/40">•</span>
              <span class="text-[11px] font-semibold text-text-main/60">{{ album.language }}</span>
            </template>
          </div>
        ...
        <Tabs :model-value="activeTab" class="w-full" @update:model-value="handleTabChange">
          <div class="song-list-sticky sticky z-[110] bg-bg-main" :style="{ top: `${tabsTop}px` }">
              ...
              <div class="flex items-center justify-between h-10">
                <TabsList class="bg-transparent border-none gap-6">
                  ...
                  <TabsTrigger value="comments" class="text-[13px]">
                    <span class="relative">评论 <Badge v-if="commentTotal > 0" :count="commentTotal" class="-right-6 scale-90" /></span>
                  </TabsTrigger>
                </TabsList>
            ...
            <SongListHeader v-if="activeTab === 'songs'" :sortField="sortField" :sortOrder="sortOrder" :showCover="true" paddingClass="px-4" @sort="handleSort" />
          </div>
      ...
      <Dialog v-model:open="showIntroDialog" title="专辑介绍" :description="album.intro" contentClass="detail-intro-dialog max-w-[720px]" descriptionClass="text-[13px]" showClose />
    </template>
  </div>
</template>
