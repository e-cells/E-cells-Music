...
const toggleFavorite = async () => {
  if (!playlist.value || !userStore.isLoggedIn || isFavoriteLoading.value) return;
  isFavoriteLoading.value = true;
  try {
    if (isFavoritePlaylist.value) {
      const success = await playlistStore.unfavoritePlaylist(playlist.value, userStore.info?.userid);
      if (!success) {
        toastStore.actionFailed('取消收藏');
      }
    } else {
      const success = await playlistStore.favoritePlaylist(playlist.value, userStore.info?.userid);
      if (!success) {
        toastStore.actionFailed('收藏歌单');
      }
    }
  } catch (error) {
    console.error('Failed to toggle favorite status', error);
    toastStore.actionFailed('收藏操作');
  } finally {
...
const secondaryActions = computed(() => {
  ...
  if (!isOwnerPlaylist.value && userStore.isLoggedIn) {
    actions.push({
      icon: isFavoritePlaylist.value ? iconHeartFilled : iconHeart, // 根据状态切换实心或空心图标
      label: isFavoritePlaylist.value ? '已收藏' : '收藏',
      emphasized: isFavoritePlaylist.value,
      tone: 'favorite',
      onTap: toggleFavorite, // 使用稳定的函数引用
